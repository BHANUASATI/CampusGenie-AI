...
import { Plus, Filter, Search, Calendar, CheckCircle, Clock, AlertTriangle, TrendingUp, Edit2, Trash2 } from 'lucide-react';
...
interface FacultyPersonalTasksProps {
  theme: 'dark' | 'light';
}

export const FacultyPersonalTasks: React.FC<FacultyPersonalTasksProps> = ({ theme }) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default:
        return theme === 'dark' ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200';
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters and Search */}
      <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-xl p-4 shadow-md`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredTasks.map(task => (
          <div key={task.id} className={`border rounded-xl p-5 transition-all duration-300 transform hover:scale-[1.02] ${getStatusColor(task.status)}`}>
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <h3 className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-black'} text-sm leading-tight mb-2`}>
                  {task.title}
                </h3>
            ...
            {task.description && (
              <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-4 leading-relaxed`}>
                {task.description}
              </p>
            )}
            
            <div className={`flex items-center justify-between text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'} mb-4`}>
              {task.due_date && (
            ...
            {task.progress > 0 && task.status !== 'completed' && (
              <div className="mb-4">
                <div className="flex items-center justify-between text-xs mb-1">
                  ...
                  <span className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>{task.progress}%</span>
                </div>
            ...
            {task.tags && task.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {task.tags.map((tag, index) => (
                  <span key={index} className={`px-2 py-1 rounded-full text-xs ${theme === 'dark' ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
                    {tag}
                  </span>
                ))}
              </div>
      ...
      {filteredTasks.length === 0 && (
        <div className="text-center py-12">
          <Calendar className={`w-16 h-16 mx-auto mb-4 ${theme === 'dark' ? 'text-gray-600' : 'text-gray-400'}`} />
          <p className={`text-lg ${theme === 'dark' ? 'text-white' : 'text-black'}`}>No tasks found</p>
          <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Create a new task to get started</p>
        </div>
      )}
...
const TaskModal: React.FC<TaskModalProps> = ({ mode, task, theme, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`${theme === 'dark' ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'} rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border shadow-2xl`}>
        <div className="p-6">
          <h2 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-black'} mb-6`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                className="rounded"
              />
              <label htmlFor="reminder_enabled" className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                Enable Reminder
              </label>
            </div>
